...
6,8,3,7,2 가 나오고 다시 앞에 나왔던 숫자들 중 같은 수인 3이 나옵니다.
...
제가 푼 방식대로라면 m(1~1,000,000)으로 나눈 나머지는 최대 1,000,000까지 나오니 배열arr을 0으로 초기화해 선언해 두고,
처음 a0=s가 주어질때 arr[s]=1(s는 1번째 나머지 라고 생각),
while문 시작-----------------------------------------------------------------------------------------------------------------
s로 다음 a1을 구합니다.(주어진 식대로)
a1을 구했으면 그 숫자가 앞서 나왔던 숫자인지 체크해보기 위해서 arr배열을 봅니다.
이때, arr[a1]==0이면 a1이라는 숫자는 앞에 나왔던 숫자가 아닌 새로운 숫자이므로 이번 턴(i,번째)를 저장합니다.(arr[a1]=i)
arr[a1]!=0면 a1이라는 숫자는 0이 아닌 어떤 번째에 있었던 숫자이므로 
