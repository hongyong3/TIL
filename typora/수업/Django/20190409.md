...
- 대신 form 가장 하단에 **다만 이 양식으로 비밀번호를 변경할 수 있습니다.** 라는 문구가 있는데 이 링크를 클릭하면 `accounts/password/` 라는 주소로 이동된다. django 가 기본적으로 설정하고 있는 링크이다. (우리가 app 이름을 accounts 로 했던 이유중 하나)
